<?php  
    ...
    $sql4 = pg_query("UPDATE cubiculos SET estado='$estado2' Where numero='$cub_anterior'");

	$sql5 = pg_query("UPDATE cubiculos SET estado='$estado' Where numero='$cubiculo'");


	$sql6 = pg_query("SELECT * FROM usuarios where id='".$_SESSION['id']."'");
    $info = pg_fetch_assoc($sql6);
    $rol=$info['rol'];

    $sql7 = pg_query("INSERT INTO historial_ordenes (norden,area_trabajo,fecha,observacion,status) VALUES ('$num_orden','$cubiculo','$fecha','$observacion','$status')");
